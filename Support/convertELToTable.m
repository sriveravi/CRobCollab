function convertELToTable( trials, file, writePermissions)
    ...
    if ~isempty( trials)
        ...
        if ~strcmp( writePermissions, 'a') % if appending, do not write header
            tableLine = num2cell( nan( 1, length(parNames)+19));   
            ...
            tableLine{i1+19} = 'message';  
            tableLine{i1+20} = 'msgTime';                          
        ...
        for triIdx = 1:length( trials )
            ...
            tableLine = num2cell( nan( 1, length(parNames)+20)); % chnged 17->20 for msg  
            for i1 = 1:length( parNames)
            ...
            for fixIdx = 1:numF
                ...
                % last message
                tableLine{i1+18} = getLastMsg( trials(triIdx).msg, trials(triIdx).msgTime, fixSac(triIdx).fixEnd(fixIdx) );
                tableLine{i1+20} = fixSac(triIdx).fixEnd(fixIdx); % end time
                % write trial info
                writeLine(fID,tableLine)
            end

            % reset table line (general info) 
            tableLine = num2cell( nan( 1, length(parNames)+20));   
            for i1 = 1:length( parNames)
                tableLine{1,i1} = trials(triIdx).( parNames{i1});
            end
            ...
            for sacIdx = 1:numS
                ...
                % last message
                tableLine{i1+18} = getLastMsg( trials(triIdx).msg, trials(triIdx).msgTime, fixSac(triIdx).sacEndTime(sacIdx) );
                tableLine{i1+20} = fixSac(triIdx).sacEndTime(sacIdx);  % end time
            ...
            % add all messages
            numMsgs = length( trials(triIdx).msg );
            for msgIdx = 1:numMsgs
                % message and time
                tableLine{i1+19} = trials(triIdx).msg{msgIdx};   %getLastMsg( trials(triIdx).msg, trials(triIdx).msgTime, fixSac(triIdx).sacStartTime(sacIdx) );
                tableLine{i1+20} = trials(triIdx).msgTime{msgIdx};
                % write trial info
                writeLine(fID,tableLine)
            end
