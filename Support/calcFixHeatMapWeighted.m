...
function [fixScape, sigma] = calcFixHeatMapWeighted( fixPos, fixDuration, imageHW, sigma )
...
for i1 = 1:length( fixDuration )
    if fixPos(1,i1) > 0 && fixPos(1,i1) < imageHW(2)
        if  fixPos(2,i1) > 0 && fixPos(2,i1) < imageHW(1)
                durationMap( fixPos(2,i1), fixPos(1,i1)) = durationMap( fixPos(2,i1), fixPos(1,i1))+fixDuration(i1); 
        end
    end
end
