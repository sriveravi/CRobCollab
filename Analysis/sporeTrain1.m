...
imSize = [ 1920, 1080]; %x,y
[width ] = imSize(1);
[height ] = imSize(2);
% imFolder = '../ExpBasic/';
imFolder = '../ChrisData/SporeStudy/Stimuli/CenteredImg/';
aoiFolder = '../ChrisData/SporeStudy/Stimuli/CenteredImg/Annotations/';
maxDist = 70;



datFldr = '../ChrisData/SporeStudy/NewAdultData1Cond/TrainingData/AllASC/';
cond = 'SporeTrainNewAdult';
...
%         end
        ...
        startMsg = 'test'; %'target'; 
        endMsg = 'responseMade';
        offsets = []; % [0, 0];
        [ stimSegment, times, ghost, timesEnd ] = cropTrialsByMsg( trials, startMsg, endMsg, offsets );

        % add trial start time and AOIs to struct
        for i1 = 1:length( times)
            % trial start time
            stimSegment(i1).trialTime = times(i1); 
            stimSegment(i1).RTEstimate = timesEnd(i1) -times(i1);
            
            %---- load AOI position, 
            imgStim = stimSegment(i1).visual(1:end-4);  %test_visual(1:end-4); % get test stim name
...
%             hold on
%             plot( stimSegment(i1).fixPos(1,:), stimSegment(i1).fixPos(2,:), 'ko', 'markersize', 10, 'markerfacecolor','r');
